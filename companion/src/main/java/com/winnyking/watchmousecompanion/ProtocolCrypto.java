 ...
 *
 * <p>Both the watch client, the Linux host (Python) and this companion derive the same session key
 * from the user PIN and a per-connection random salt sent by the receiver:
 *
...
public final class ProtocolCrypto {
    ...
    public static byte[] hkdfExpand(byte[] prk, byte[] info, int length) {
        ...
        int counter = 1;
        while (offset < length) {
            byte[] block = hmac(prk, concat(previous, info, new byte[] {(byte) counter}));
